...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Send the real email via Gmail SMTP (PHPMailer).
    require_once __DIR__ . '/lib/mailer.php';
    [$sent, $error] = send_mail($demande['email'], $demande['nom'] ?? '', $subject, $content);

    // Log it in the emails history regardless of send outcome.
    $insert = $pdo->prepare('INSERT INTO emails (id_demande, sujet, contenu) VALUES (?, ?, ?)');
    $insert->execute([$idDemande, $subject, $content]);

    log_action($pdo, 'Email ' . ($sent ? 'envoye' : 'tente') . ' pour la demande #' . $idDemande . ': ' . $subject);
    flash(
        $sent ? 'success' : 'danger',
        $sent
            ? 'Email envoyé à ' . $demande['email'] . ' et enregistré.'
            : "Email enregistré mais NON envoyé : " . ($error ?? 'erreur inconnue')
    );
