...
// ── Visitor stats (from the shared `page_visits` table) ──────────────
// visited_at <- date_visite, session_id <- unique visitor (was ip_hash).
$visitsToday  = (int) $pdo->query("SELECT COUNT(*)                  FROM page_visits WHERE DATE(visited_at) = CURDATE()")->fetchColumn();
$visitsWeek   = (int) $pdo->query("SELECT COUNT(*)                  FROM page_visits WHERE visited_at >= DATE_SUB(NOW(), INTERVAL 7  DAY)")->fetchColumn();
$visitsMonth  = (int) $pdo->query("SELECT COUNT(*)                  FROM page_visits WHERE visited_at >= DATE_SUB(NOW(), INTERVAL 30 DAY)")->fetchColumn();
$uniqueToday  = (int) $pdo->query("SELECT COUNT(DISTINCT session_id) FROM page_visits WHERE DATE(visited_at) = CURDATE()")->fetchColumn();
$uniqueWeek   = (int) $pdo->query("SELECT COUNT(DISTINCT session_id) FROM page_visits WHERE visited_at >= DATE_SUB(NOW(), INTERVAL 7  DAY)")->fetchColumn();
$uniqueMonth  = (int) $pdo->query("SELECT COUNT(DISTINCT session_id) FROM page_visits WHERE visited_at >= DATE_SUB(NOW(), INTERVAL 30 DAY)")->fetchColumn();

// Visits per day – last 30 days
$rawDays = $pdo->query("
    SELECT DATE(visited_at) AS jour,
           COUNT(*)                  AS total,
...
$rawHours = $pdo->query("
    SELECT HOUR(visited_at) AS h, COUNT(*) AS n
    FROM page_visits WHERE DATE(visited_at) = CURDATE()
    GROUP BY h
")->fetchAll();
